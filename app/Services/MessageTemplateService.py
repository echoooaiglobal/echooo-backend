...
class MessageTemplateService:
    ...
    async def create_template(template_data: Dict[str, Any], created_by: uuid.UUID, db: Session):
        ...
        Returns:
            MessageTemplate: The created message template
        """
        try:
            # Set creator
    ...
    async def delete_template(template_id: uuid.UUID, db: Session):
        ...
        try:
            ...
            from app.Models.campaign_models import InfluencerList
            lists_using_template = db.query(InfluencerList).filter(
                InfluencerList.message_template_id == template_id
            ).count()
